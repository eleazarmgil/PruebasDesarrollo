...
public class ConsultarOpcionDePagoPorIdControllerTest
{
    ...
    public ConsultarOpcionDePagoPorIdControllerTest()
    {
    ...
    [Fact(DisplayName = "ConsultarOpcionDePagoPorIdController estatus 200-ok")]
    public async Task ConsultarOpcionDePagoPorIdStatus200OK()
    {
        //Arrange-> Datos necesario para las pruebas
        var request = BuildDataContextFaker.BuildConsultarOpcionDePagoPorIdRequest();
        var valores = BuildDataContextFaker.BuildConsultarOpcionDePagoPorIdResponse();

        _mediatorMock.Setup(x => x.Send(It.IsAny<ConsultarOpcionDePagoPorIdQuery>(), It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(valores));

        //Act-> Cuales son las actividades de mi consulta que debo tener
        var result = await _controller.ConsultarOpcionDePagoPorId(request);
    ...
    [Fact(DisplayName = "ConsultarOpcionDePagoPorIdController estatus 400-BadRequest")]
    public async Task ConsultarOpcionDePagoPorIdStatus400BadRequestTest()
    {
        //Arrange-> Datos necesario para las pruebas
        var request = BuildDataContextFaker.BuildConsultarOpcionDePagoPorIdRequest();
        _mediatorMock.Setup(x => x.Send(It.IsAny<ConsultarOpcionDePagoPorIdQuery>(), It.IsAny<CancellationToken>()))
