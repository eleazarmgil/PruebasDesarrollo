...
public class AgregarRegistrarPrestadorCommandHandler : IRequestHandler<AgregarRegistrarPrestadorCommand, Guid>
{
    ...
    public async Task<Guid> Handle(AgregarRegistrarPrestadorCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                _logger.LogWarning("AgregarRegistrarPrestadorCommandHandler.Handle: Request nulo.");
            ...
            else
            {
                ...
                //Llamo a validator del Agregar Prestador y verifico 
                if (result.IsValid)
                {
        ...
        {
            _logger.LogWarning("AgregarRegistrarPrestadorCommandHandler.Handle: ValidationException ");
            throw;
        }
        catch (Exception)
        {
    ...
    private async Task<Guid> HandleAsync(AgregarRegistrarPrestadorCommand request)
    {
        ...
        try
        {
            ...
            var result = _dbContext.Prestador.Count(c => c.usuario == request._request.usuario || c.rif == request._request.rif);
