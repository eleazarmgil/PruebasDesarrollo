...
using UCABPagaloTodoMS.Application.Validators.Servicios;
...
public class ActualizarServicioCommandHandler : IRequestHandler<ActualizarServicioCommand, Guid>
{
    ...
    public async Task<Guid> Handle(ActualizarServicioCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var validator = new ActualizarServicioValidator();
        ...
        {
            _logger.LogWarning("ActualizarServicioCommandHandler.Handle: ValidationException ");
            throw;
        }
        catch (Exception)
        {
